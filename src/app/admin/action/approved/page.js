...
const Page = ({ searchParams }) => {
  ...
  const fetchData = async () => {
    const url = `${process.env.NEXT_PUBLIC_HOST}/api/blogpost/approvedblogs?currentpage=${currentPage}&datalimit=${dataLimit}`;
  ...
  const nameThree = "approved";
  const linkThree = `${process.env.NEXT_PUBLIC_FRONTEND_LINK}/admin/action/approved`;
  const myPage = "approved";
  useEffect(() => {
    fetchData();
    tokenVerify();
  }, []);
