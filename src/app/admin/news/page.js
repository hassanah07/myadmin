...
const Page = ({ searchParams }) => {
  ...
  const fetchData = async () => {
    const url = `${process.env.NEXT_PUBLIC_HOST}/api/blogpost/news?currentpage=${currentPage}&datalimit=${dataLimit}`;
  ...
  const nameThree = "news";
  const linkThree = `${process.env.NEXT_PUBLIC_FRONTEND_LINK}/admin/news`;
  const myPage = "news";
  useEffect(() => {
    fetchData();
    tokenVerify();
